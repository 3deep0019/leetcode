...
export function getProblemNumber(folderName) {
  const match = folderName.match(/^(\d+)-/);
  return match ? parseInt(match[1], 10) : Number.MAX_SAFE_INTEGER;
}

export function parseDifficulty(readme) {
...
export function extractTitle(readme) {
  const match = readme.match(/<h2>\s*<a[^>]*>([^<]*)<\/a>\s*<\/h2>/i);
  if (match) return match[1].trim();
  return null;
}

export function extractUrl(readme) {
  const match = readme.match(/<h2>\s*<a[^>]*href="([^"]*)"[^>]*>/i);
  return match ? match[1] : null;
}

export function findSolutionFile(dir) {
  const jsFiles = fs
    .readdirSync(dir)
...
export function getProblemFolders(root) {
  return fs
    ...
    .filter(
      (entry) =>
        ...
        !["compiled", "node_modules", "dist", "src", "lib", "scripts", "public"].includes(
          entry.name
        ) &&
        /^\d+-/.test(entry.name)
    )
...
export function extractFunctionName(solution) {
...
export function usesListNode(solution, readme) {
  if (/@param\s+\{ListNode\}/i.test(solution)) return true;
  if (/@return\s+\{ListNode\}/i.test(solution)) return true;
  if (/\b(?:l1|l2|list1|list2)\s*=\s*\[/i.test(readme)) return true;
  return false;
}

export function mutatesInput(solution) {
...
function readValue(str, pos) {
  ...
  if (literalMatch) {
    ...
    const value = token === "true" ? true : token === "false" ? false : Number(token);
...
export function parseExamples(readme) {
  const examples = [];
...
export function extractSignatureStub(solution, functionName) {
...
export function buildProblemSection(folderName, readme, solution, notes) {
  const sections = [
    `## ${folderName}`,
    "",
    "### Problem",
...
export function collectProblems(root) {
  ...
  for (const folderName of getProblemFolders(root)) {
    ...
    const title = extractTitle(readme) || folderName;
    const url = extractUrl(readme);
...
export function toUiProblem(problem, difficulty) {
  let description = htmlToPlainText(problem.readme);
  const lines = description.split("\n");
  if (lines[0] === problem.title) lines.shift();
  if (lines[0] === problem.url) lines.shift();
  while (lines[0] === "") lines.shift();
  description = lines.join("\n");

  return {
    id: problem.folderName,
    number: getProblemNumber(problem.folderName),
    title: problem.title,
    difficulty,
    url: problem.url,
    description,
    functionName: problem.functionName,
    starterCode: problem.signatureStub,
